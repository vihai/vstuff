...
struct q931_ie_connected_number *q931_ie_connected_number_alloc()
{
...
struct q931_ie *q931_ie_connected_number_alloc_abstract()
{
	return &q931_ie_connected_number_alloc()->ie;
}
	...
	int len)
{
	...
	if (!oct_3->ext) {
		...
		ie->presentation_indicator =
			Q931_IE_CN_PI_PRESENTATION_ALLOWED;
		ie->screening_indicator =
			Q931_IE_CN_SI_USER_PROVIDED_NOT_SCREENED;
	}
...
{
	struct q931_ie_connected_number *ie =
		container_of(generic_ie, struct q931_ie_connected_number, ie);
	...
	enum q931_ie_connected_number_type_of_number type_of_number)
{
	...
	case Q931_IE_CN_TON_NATIONAL:
		return "National";
	case Q931_IE_CN_TON_NETWORK_SPECIFIC:
		return "Network specific";
	case Q931_IE_CN_TON_SUBSCRIBER:
		return "Subscriber";
	case Q931_IE_CN_TON_ABBREVIATED:
		return "Abbreviated";
	case Q931_IE_CN_TON_RESERVED_FOR_EXT:
		return "Reserved";
	default:
		return "*INVALID*";
	}
		...
		numbering_plan_identificator)
{
	...
	case Q931_IE_CN_NPI_DATA:
		return "Data";
	case Q931_IE_CN_NPI_TELEX:
		return "Telex";
	case Q931_IE_CN_NPI_NATIONAL_STANDARD:
		return "National standard";
	case Q931_IE_CN_NPI_PRIVATE:
		return "Private";
	case Q931_IE_CN_NPI_RESERVED_FOR_EXT:
		return "Reserved";
	default:
		return "*INVALID*";
	}
		...
		presentation_indicator)
{
	...
	case Q931_IE_CN_PI_NOT_AVAILABLE:
		...
		screening_indicator)
{
	...
	case Q931_IE_CN_SI_USER_PROVIDED_VERIFIED_AND_FAILED:
		return "User provided, verified and failed";
	case Q931_IE_CN_SI_NETWORK_PROVIDED:
	...
	const char *prefix)
{
	...
	report(LOG_DEBUG, "%sType of number = %s (%d)\n", prefix,
		q931_ie_connected_number_type_of_number_to_text(
			ie->type_of_number),
		ie->type_of_number);
	...
	report(LOG_DEBUG, "%sPresentation indicator = %s (%d)\n", prefix,
		q931_ie_connected_number_presentation_indicator_to_text(
			ie->presentation_indicator),
		ie->presentation_indicator);

	report(LOG_DEBUG, "%sScreening indicator = %s (%d)\n", prefix,
		q931_ie_connected_number_screening_indicator_to_text(
			ie->screening_indicator),
		ie->screening_indicator);
