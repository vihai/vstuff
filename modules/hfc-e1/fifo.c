...
{
	if (fifo->enabled && fifo->chan->connected_e1_chan) {

		if (fifo->direction == RX)
			return TRUE;
		else if ((fifo->chan->connected_e1_chan->port->nt_mode &&
		    fifo->chan->connected_e1_chan->port->l1_state == 1) ||
		   (!fifo->chan->connected_e1_chan->port->nt_mode &&
		    fifo->chan->connected_e1_chan->port->l1_state == 1))
			return TRUE;
	}
