	...
	char *buf)
{
	...
	for (i=0; i<3; i++) {
		...
		if (fifo_rx->connected_chan) {
			len += snprintf(buf + len, PAGE_SIZE - len,
				" st:%s",
				fifo_rx->connected_chan->chan->visdn_chan.name);
		}

		if (fifo_tx->connected_chan) {
			len += snprintf(buf + len, PAGE_SIZE - len,
				" st:%s",
				fifo_tx->connected_chan->chan->visdn_chan.name);
		}
...
{
	if (fifo->connected_chan &&
	    fifo->connected_chan->chan->queue_stopped)
		visdn_leg_wake_queue(
			&fifo->connected_chan->chan->visdn_chan.leg_a);
}
