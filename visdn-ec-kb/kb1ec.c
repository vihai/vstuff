 ...
 *
 * A pdf of which is available by searching on the document title at
 * http://www.ti.com/
 *
 */
...
/* Uncomment to provide summary statistics for overall echo can
 * performance every 4000 samples
 */
...
/* Uncomment to generate per-sample statistics - this will severely
 * degrade system performance and audio quality
 */
...
typedef struct {
	...
	int size_d;			 
	/* The actual sample -  twice as large as we need, however we do
	 * store values at idx_d and idx_d+size_d
	 */
	short *buf_d;			
} echo_can_cb_s;
...
typedef struct
{
	/* an arbitrary ID for this echo can - this really should be
	 * settable from the calling channel...
	 */
	int id;

	/* absolute time - aka. sample number index - essentially the
	 * number of samples since this can was init'ed
	 */
	int i_d;
	 ...
	 * -----------------------------------
	 * reference signal power estimate -
	 * aka. Average absolute value of y(k)
	 */
	int Ly_i;			
	/* ... */
	int Lu_i;
	 ...
	 * ---------------------------------
	 * Power estimate of the recent past of the near-end hybrid signal
	 * - aka. Short-time average of: 2 x |s(i)|
	 */
	int s_tilde_i;		
	/* Power estimate of the recent past of the far-end receive signal
	 * - aka. Short-time average of:     |y(i)|
	 */
	int y_tilde_i;

	/* Near end speech detection counter - stores Hangover counter time
	 * remaining, in samples
	 */
	int HCNTR_d;			
	...
	echo_can_cb_s u_s;
	/* Reference samples of far-end receive signal used to calculate
	 * short-time average
	 */
...
static void init_cb_s(echo_can_cb_s *cb, int len, void *where)
{
...
static void add_cc_s(echo_can_cb_s *cb, short newval)
{
	/* Can't use modulus because N+M isn't a power of two (generally) */
	cb->idx_d--;
	if (cb->idx_d < 0) 
		/* Whoops - the pointer to the 'start' wrapped around so
		 * reset it to the top of the buffer
		 */
	 	cb->idx_d += cb->size_d;
...
static short get_cc_s(echo_can_cb_s *cb, int pos)
{
	/* Load two copies into memory */
	return cb->buf_d[cb->idx_d + pos];
}

static void init_cc(echo_can_state_t *ec, int N, int maxy, int maxu) 
{
...
short echo_can_update(echo_can_state_t *ec, short iref, short isig) 
{
	...
	ec->y_tilde_i -= abs(get_cc_s(&ec->y_s,
				(1 << DEFAULT_ALPHA_YT_I) - 1 ))
					>> DEFAULT_ALPHA_YT_I;
	...
	/* ------------------------------------------------------------------- */
	/* Delete the oldest sample from the power estimate accumulator */
	ec->s_tilde_i -= abs(get_cc_s(&ec->s_s,
				(1 << DEFAULT_ALPHA_ST_I) - 1 ));
	/* Add the new sample to the power estimate accumulator */
	...
	/* Push a copy of the current short-time average of the far-end
	 * receive signal into it's circular buffer
	 */
	add_cc_s(&ec->y_tilde_s, ec->y_tilde_i);		

	/* flow B on pg. 428 */
  
	/* If the hangover timer isn't running then compute the new
	 * convergence factor, otherwise set Py_i to 32768
	 */
	if (!ec->HCNTR_d) {
		Py_i = (ec->Ly_i >> DEFAULT_SIGMA_LY_I) *
	 ...
	 * Do not do this for the first (DEFAULT_UPDATE_TIME) secs after
	 * speech
	...
	if (ec->y_tilde_i > ec->max_y_tilde) {
		...
		ec->max_y_tilde = MAX16(ec->y_tilde_s.buf_d +
					ec->y_tilde_s.idx_d,
					ec->N_d,
					&ec->max_y_tilde_pos);
	}
	...
	if (((ec->s_tilde_i >> (DEFAULT_ALPHA_ST_I - 1)) > ec->max_y_tilde) 
		...
		printk(KERN_INFO
			"Reset near end speech timer with: "
			"s_tilde_i %d, "
			"stmnt %d, "
			"max_y_tilde %d\n",
			ec->s_tilde_i,
			(ec->s_tilde_i >> (DEFAULT_ALPHA_ST_I - 1)),
			ec->max_y_tilde);
#endif
#ifdef MEC2_STATS
		++ec->cntr_nearend_speech_frames;
#endif
	} else if (ec->HCNTR_d > 0) {
  		/* otherwise, if it's still non-zero, decrement the"
		 * Hangover counter by one sample
		 */
#ifdef MEC2_STATS
		++ec->cntr_nearend_speech_frames;
#endif
	...
	/* Update coefficients if no near-end speech in this sample
	 * (ie. HCNTR_d = 0)
	...
	if (!ec->HCNTR_d &&
		/* no near-end speech present */
		!(ec->i_d % DEFAULT_M)) {
			/* we only update on every DEFAULM_M'th sample
			 * from the stream
			 */
  			if (ec->Lu_i > MIN_UPDATE_THRESH_I) {
				/* there is sufficient energy above the
				 * noise floor to contain meaningful data
  				 * so loop over all the filter coefficients */
#ifdef MEC2_STATS_DETAILED
				printk(KERN_INFO
					"updating coefficients with: "
					"ec->Lu_i %9d\n",
					ec->Lu_i);
#endif
#ifdef MEC2_STATS
...
#ifdef MEC2_STATS_DETAILED
				printk(KERN_INFO
					"insufficient signal to update "
					"coefficients ec->Lu_i %5d < %5d\n",
	...
	if (ec->HCNTR_d == 0)
		printk(KERN_INFO
			"possibily correcting frame with ec->Ly_i %9d"
			" ec->Lu_i %9d and expression %d\n",
...
#ifdef MEC2_STATS_DETAILED
		printk(KERN_INFO
			"aggresively correcting frame with ec->Ly_i %9d"
			" ec->Lu_i %9d expression %d\n",
			ec->Ly_i, ec->Lu_i, (ec->Ly_i/(ec->Lu_i + 1)));
#endif
#ifdef MEC2_STATS
...
#ifdef MEC2_STATS_DETAILED
			printk(KERN_INFO
				"correcting frame with ec->Ly_i %9d "
				"ec->Lu_i %9d expression %d\n",
				ec->Ly_i, ec->Lu_i,
				(ec->Ly_i/(ec->Lu_i + 1)));
#endif
	...
	if ((ec->i_d % MEC2_STATS) == 0) {
		...
		if (ec->cntr_coeff_missedupdates > 0)
			ec->avg_Lu_i_toolow =
				(int)(ec->avg_Lu_i_toolow /
					ec->cntr_coeff_missedupdates);
		else
			ec->avg_Lu_i_toolow = -1;

		if (ec->cntr_coeff_updates > 0)
			ec->avg_Lu_i_ok = (ec->avg_Lu_i_ok /
						ec->cntr_coeff_updates);
		else
			ec->avg_Lu_i_ok = -1;
		...
		printk(KERN_INFO
			"%d: "
			"Near end speech: %5d "
			"Residuals corrected/skipped: %5d/%5d "
			"Coefficients updated ok/low sig: %3d/%3d "
			"Lu_i avg ok/low sig %6d/%5d\n", 
			ec->id,
			ec->cntr_nearend_speech_frames, 
			ec->cntr_residualcorrected_frames,
			ec->cntr_residualcorrected_framesskipped, 
...
echo_can_state_t *echo_can_create(int len, int adaption_mode)
{
	...
	ec = (echo_can_state_t *)MALLOC(eclen);
	if (!ec)
		return NULL;
...
int echo_can_traintap(echo_can_state_t *ec, int pos, short val)
{
